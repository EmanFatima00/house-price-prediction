...
def format_pkr(amount):
    """Format amount in simple PKR with commas"""
...
with c2: st.markdown(f'<div class="metric-card"><h3>{format_pkr(meta["mae"])}</h3><p>Mean Abs Error</p></div>', unsafe_allow_html=True)
        ...
        if predict_btn:
            ...
            <div class="result-box">
                ...
                <h1 style="color:#6366F1;font-size:2.8rem;margin:0;">
                    {format_pkr(pred_price)}
                </h1>
                <p style="color:#64748B;margin-top:6px;">
                    Range: {format_pkr(low_est)} — {format_pkr(high_est)}
                </p>
            </div>""", unsafe_allow_html=True)
